...
if (isset($_POST['selectedDate'])) {
    ...
    $selectuser = "SELECT user.First_name, user.last_name, user.user_id,user.email, user.user_name, profile.location, profile.birthdate
                   FROM user
                   INNER JOIN profile ON user.user_id = profile.user_id
                   WHERE profile.birthdate = '$selectedDate' and user.Role='user'";
